...
year_1981 <- filter(surveys, "year" == 1981) ##nope no effect
...
# Add other columns
surveys <- surveys %>%
  mutate(., weight_kg2 = weight/1000, weight_kg*2)
str(surveys)
...
#Return a table with three columns: year, weight of the heaviest 
#animal in grams, and weight in kilograms, arranged (arrange()) in 
#descending order, from heaviest to lightest. (This table should have 26 rows, one for each year)
